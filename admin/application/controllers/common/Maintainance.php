...
class Maintainance extends AdminController {
	...
	public function getList() {
	    ...
	    if ($this->hasPermission('access', $this->uri->segment('1'))) {
    	    ...
    	    $data['maintainances'] = $this->maintainance_model->getMaintainances();
	...
	public function add() {
	    ...
	    if (!empty($_POST)) {
	        ...
	        if ($this->form_validation->run() == false) {
	            ...
	            $result = $this->maintainance_model->addMaintainance($_POST, $this->global ['userId']);
	            if(count($result) > 0) {
