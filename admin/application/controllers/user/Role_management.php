...
class Role_management extends AdminController {
    ...
    public function getList() {
        ...
        if ($this->hasPermission('access', $this->uri->segment('1'))) {
            ...
            $results = $this->user_model->getUserRoles();
    ...
    public function add() {
        ...
        if (!empty($_POST)) {
            ...
            if($msg == '') {
                $this->user_model->addRoles($_POST, $this->global ['userId']);
    ...
    public function edit($b64_ugid = NULL) {
        ...
        if (!empty($_POST)) {
            ...
            if (empty($data['user_group_name'])){
                ...
                $roledata= $this->user_model->getUserGroup($user_group_id);
                $rolename= $this->user_model->getTotalRole($data['user_group_name']);
                //print_r($postname->total);
            ...
            if($msg == '') {
                $this->user_model->editRoles($_POST, $user_group_id, $this->global ['userId']);
                $this->session->set_flashdata('success', $text_success);
                redirect(base_url().'roles');
            } else {
                $this->session->set_flashdata('error', $msg);
    ...
    public function getForm($b64_ugid = NULL) {
        ...
        if ($this->hasPermission('access', $this->uri->segment('1'))) {
    	    ...
    	    if($user_group_id == '') {
    	        ...
    	        $user_group = $this->user_model->getUserGroup($user_group_id);
