...
class Commission_model extends CI_Model {
    ...
    function getCommissionAll($customer_id) {
        $query = $this->db->query("SELECT *,c.status as comm_status FROM `commission` as c, `customer` as ct WHERE c.trainer_id = ct.customer_id and c.trainer_id='".$customer_id."' order by commission_id desc");
        $result = $query->result();
        return $result;
    }
