...
<script>
	function addUser() {
		...
		$.ajax({
			...
			method: 'POST',
			data: {first_name: first_name, last_name: last_name, username: username, emp_id: emp_id, email_id: email_id, phone_no: phone_no, gender: gender, usergroup_id: usergroup_id, joining_date: joining_date},
			dataType: 'json',	
			success: function(json){
				if(json['success']) {
						
					document.getElementById("demo").innerHTML = json['msg'];  
				} else {
					...
					if(json['err_username']) {
						$("#username_err").html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>' + json['err_username'] + '</div>').show('fast').delay(5000).hide('fast');
					}
					if(json['err_empid']) {
						$("#empid_err").html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>' + json['err_empid'] + '</div>').show('fast').delay(5000).hide('fast');
					}
					if(json['err_emailid']) {
